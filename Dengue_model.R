...
# Formating data
require(tidyverse)
colnames(dengueERJ) <- c("DT_NOTIF","SEM_NOTIF","ANO_NOTIF","DT_SIN_PRI","SEM_SIN_PRI","DT_DIGITA","MUNIC_GEOCODIG","CID10_CODIG")
...
d <- d[d$ANO_NOTIF != 2010 & d$ANO_NOTIF != 2013 & d$ANO_NOTIF != 2014, ]
...
# Getting the most recent week
Tactual <- as.numeric(substr(episem(x= as.Date(max(d$DT_NOTIF), format="%Y-%m-%d")),5,6)) #68
...
# Observed data
delay.data.obs <- delay.data[,(0:Dmax)+1]

# Time index of the unknown counts (Dmax+1,...,Tactual) 
index.time <- (nrow(delay.data.obs)-Dmax+1):nrow(delay.data.obs)

delay.data.obs.trian <- delay.data.obs

# Creating the run-off triangle data frame
delay.data.obs.trian[outer(1:nrow(delay.data.obs.trian), 0:Dmax, FUN = "+") > nrow(delay.data.obs.trian)] <- NA
...
## Model Checking ##
####################
y.samples <- do.call('rbind',lapply(X = delay.samples.list, FUN = function(x) rnbinom(nrow(delay.data.obs.trian)*(Dmax+1), mu = exp(x$latent), size = x$hyperpar[1]))) 

y.array <- array(y.samples,dim=c(n.sim,nrow(delay.data.obs.trian),Dmax+1))
  
total.samples <- rowSums(y.array,dims=2)

true.totals <- delay.tbl$Notifications[1:nrow(delay.data.obs.trian)]
...
# Prediction plot
plot(delay.tbl$Notifications[1:nrow(delay.data.obs.trian)], type = "n", axes=T, xlab="Time (weeks)", ylab="Number of dengue cases",ylim=c(0,18000))
lines(delay.tbl$Notifications[1:nrow(delay.data.obs.trian)], lwd=4)
...
for(i in 1:8){
  plot(density(apply(log(abs(jitter(total.samples))),1,function(x)autocor(x,i))),xlab=paste('Sample Auto-correlation Lag-',i,sep=''),main='')
...
for(i in 1:(Dmax+1)){
  ...
  abline(v=mean(delay.data.obs[1:nrow(delay.data.obs.trian),i]))
  title(paste('Delay',i-1,': p =',
              mean(apply(y.array[,,i],1,mean)<=mean(delay.data.obs[1:nrow(delay.data.obs.trian),i]))))
}
...
for(j in 1:(Dmax+1)){
  for(i in 1:(Dmax+1)){
      cov_matrix[i,j] <- mean(apply(cbind(y.array[,,i],y.array[,,j]),1,
                                 function(x)cov(x[1:nrow(delay.data.obs.trian)],x[(nrow(delay.data.obs.trian)+1):(2*nrow(delay.data.obs.trian))]))<=
                             cov(delay.data.obs[1:nrow(delay.data.obs.trian),i],delay.data.obs[1:nrow(delay.data.obs.trian),j]))
  }
}
...
#x11(width=9,height=6)
par(mar = c(4, 4, 1, 1),cex=1.2)
plot(delay.tbl$Notifications[1:nrow(delay.data.obs.trian)], type = "n", axes=F, xlab="Time (in epidemic weeks)", ylab="Number of dengue cases")
  polygon(
  ...
  axis(2)
  axis(1, at = c(2, 54 , 106) ,labels = c(min(levels(d$ANO_NOTIF)):max(levels(d$ANO_NOTIF)))[-c(1,length(levels(d$ANO_NOTIF)))] )
  points(x=nrow(delay.data.obs.trian), y=-0, pch = 10, cex=2)
  legend("topright", c("Eventually reported cases", "Currently reported cases", "Model predictions"), lty=c(1,2,3), col=c("black","red","black"), lwd=4)
dev.print(pdf,"15thWeek.pdf")

#Same graph but emphasizing last 100 observations
...
par(mfrow=c(3,4))
for(d in 1:(Dmax+1)){
  plot.inla.re(output$summary.random$tt[((d-1)*nrow(delay.data.obs.trian)+1):(d*nrow(delay.data.obs.trian)),])
...
for(Tactual in nrow(delay.data.obs.trian):(nrow(delay.data.obs.trian)+Dmax)){
  ...
  plot(delay.tbl$Notifications, type = "l", xlab="", ylab="",main=bquote(N[.(Tactual)]))
  polygon(
    x = c(index.time, rev(index.time)), 
    y = c(apply(predictions.samples,1,quantile,probs = 0.025), 
          rev(apply(predictions.samples,1,quantile,probs = 0.975))),
  ...
  lines(delay.tbl$Notifications, lwd=4)
  lines(index.time, rowMeans(predictions.samples), col="black", lwd=4, lty=3)
  lines(index.time, rowSums(delay.data.obs.trian[index.time,],na.rm = T), lwd=4, col="red", lty=2)
